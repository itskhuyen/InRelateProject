...
test_conditions<-3
filename1<-"output1.csv" #check what ouput file
#ask for dummy dataset

loglikibd<-function(ibds) {
# array has to be of dim=c(1, number of loci)
loglik<-array(0,dim=c(1,number_of_loci))
for(l in 1:number_of_loci){
sumibdloci<-0
for(i in 1:9){
...
predictors<-array(0,dim=c(number_of_loci,9)) #9 different ibs?
